  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl ring-1 ring-black/5 p-5 space-y-4 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
        ...
        {/* Address */}
        <AddressAutocomplete value={form.address} onChange={(val: string) => setForm((p: any) => ({ ...p, address: val }))} onSelect={(addr, city, state, zip, country) => setForm((p: any) => ({ ...p, address: addr, city, state, zip, country: country || p.country }))} placeholder={t('quote.address')} />
        ...
        <div>
          ...
          <div className="space-y-2">
            {[
              ...
              { key: 'urgent', color: 'bg-amber-500', border: 'border-amber-300', bg: 'bg-amber-50', text: 'text-amber-700', label: t('quote.urgent'), sub: t('quote.urgentSub') },
              { key: 'emergency', color: 'bg-red-500', border: 'border-red-300', bg: 'bg-red-50', text: 'text-red-700', label: t('quote.emergency'), sub: t('quote.emergencySub') },
            ].map((item) => {
              const sel = form.urgency === item.key;
              return (
                <button
                  key={item.key}
                  type="button"
                  onClick={() => setForm((p: any) => ({ ...p, urgency: item.key }))}
