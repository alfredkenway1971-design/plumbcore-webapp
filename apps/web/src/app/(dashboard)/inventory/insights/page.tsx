...
export default function InventoryInsightsPage() {
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-foreground">Inventory Insights</h1>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Items" value={totalItems} color="text-foreground" sub="Unique SKUs in stock" />
        <StatCard label="Total Value" value={`$${totalValue.toLocaleString()}`} color="text-primary" sub="At current unit prices" />
        <StatCard
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-muted-foreground">Item</th>
                  <th className="px-3 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground">Current Qty</th>
                  <th className="px-3 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground">Min Stock</th>
                  <th className="px-3 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground">Suggested Order</th>
              ...
              <tbody>
                {lowStockItems.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="border-b border-border transition-colors hover:bg-white/[0.02]">
                      <td className="px-3 py-3 text-foreground font-medium">{item.name}</td>
                      <td className="px-3 py-3 text-right font-semibold text-red-600">{item.quantity}</td>
                      <td className="px-3 py-3 text-right text-muted-foreground">{item.minQuantity}</td>
                      <td className="px-3 py-3 text-right text-foreground font-medium">{suggestedOrder}</td>
      ...
      <Card variant="bordered" padding="md">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-semibold text-foreground">Overstock Items</h2>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="px-3 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground">Current Qty</th>
                  <th className="px-3 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground">Min Stock</th>
                  <th className="px-3 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground">Excess</th>
                  <th className="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wider text-muted-foreground">Suggested Action</th>
                </tr>
              </thead>
              <tbody>
                {overstockItems.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="border-b border-border transition-colors hover:bg-white/[0.02]">
                      <td className="px-3 py-3 text-foreground font-medium">{item.name}</td>
                      <td className="px-3 py-3 text-right text-foreground font-medium">{item.quantity}</td>
                      <td className="px-3 py-3 text-right text-muted-foreground">{item.minQuantity}</td>
                      <td className="px-3 py-3 text-right text-amber-400 font-semibold">+{excess}</td>
                      <td className="px-3 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button className="rounded-xl bg-white/5 px-2.5 py-1 text-[10px] font-medium text-muted-foreground/80 hover:bg-white/10 hover:text-foreground transition-colors">
                            Reduce Price
                          </button>
                          <button className="rounded-xl bg-white/5 px-2.5 py-1 text-[10px] font-medium text-muted-foreground/80 hover:bg-white/10 hover:text-foreground transition-colors">
                            Return to Supplier
      ...
      <Card variant="bordered" padding="md">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-foreground">AI Inventory Analysis</h2>
          </div>
          <Button
            size="sm"
        ...
        {!insightsLoaded && !aiRunning ? (
          <div className="rounded-xl border border-dashed border-white/10 bg-white/[0.02] p-8 text-center">
            <svg className="mx-auto h-12 w-12 text-muted-foreground/40 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M9.75 3.104v5.714a2.25 2.25 0 01-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 014.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0112 15a9.065 9.065 0 00-6.23.693L5 14.5m14.8.8l1.402 1.402c1.232 1.232.65 3.318-1.067 3.611A48.309 48.309 0 0112 21c-2.773 0-5.491-.235-8.135-.687-1.718-.293-2.3-2.379-1.067-3.61L5 14.5" />
            </svg>
            <p className="text-sm text-muted-foreground/80">Click "Run Analysis" to generate AI-powered inventory insights based on your current stock levels and usage patterns.</p>
          </div>
        ) : aiRunning ? (
            ...
            {insights.map((insight, index) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground/80 leading-relaxed">{insight.description}</p>
                </div>
              </div>
            ))}
          </div>
