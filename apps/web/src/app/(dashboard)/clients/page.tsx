      ...
      <div className="relative">
        ...
        <input
          ...
          className={`w-full h-11 pl-10 pr-4 bg-white border rounded-xl text-sm text-slate-900 placeholder:text-slate-400 outline-none transition-all ${
            error ? 'border-red-300 focus:ring-2 focus:ring-red-100' : 'border-slate-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-100'
          }`}
        />
      </div>
        ...
        <div className="absolute z-50 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden">
          {suggestions.map((s, i) => (
            ...
            >
              ...
              {s.address?.city && <p className="text-xs text-slate-400 mt-0.5">{s.address.city}, {s.address.state} {s.address.postcode}</p>}
            </button>
          ))}
        </div>
      )}
...
function SkeletonRow() {
  return <tr className="border-b border-slate-100">{Array.from({ length: 9 }).map((_, i) => <td key={i} className="py-3.5 px-4"><div className="animate-pulse h-4 rounded bg-slate-100 w-3/4" /></td>)}</tr>;
}
...
const typeColors: Record<string, string> = { residential: 'bg-blue-50 text-blue-600', commercial: 'bg-amber-50 text-amber-600' };
    ...
    <div className="max-w-[1440px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">{list.length} total clients</p>
        </div>
        <button onClick={() => { resetForm(); setShowModal(true); }} className="flex items-center gap-1.5 h-10 px-4 rounded-xl bg-blue-500 text-white text-sm font-semibold hover:bg-blue-600 transition-colors shadow-sm">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 text-xs font-semibold text-slate-400 uppercase tracking-wider">
                    <th className="py-3.5 px-4 text-left cursor-pointer select-none hover:text-slate-700" onClick={() => toggleSort('name')}>Name {sortIcon('name', sortField, sortDir)}</th>
                    <th className="py-3.5 px-4 text-left">Phone</th>
                    <th className="py-3.5 px-4 text-left">Email</th>
                    <th className="py-3.5 px-4 text-left cursor-pointer select-none hover:text-slate-700 hidden md:table-cell" onClick={() => toggleSort('city')}>City {sortIcon('city', sortField, sortDir)}</th>
                    <th className="py-3.5 px-4 text-left hidden lg:table-cell">Tags</th>
                    <th className="py-3.5 px-4 text-right cursor-pointer select-none hover:text-slate-700" onClick={() => toggleSort('totalJobs')}>Jobs {sortIcon('totalJobs', sortField, sortDir)}</th>
                    <th className="py-3.5 px-4 text-right cursor-pointer select-none hover:text-slate-700 hidden sm:table-cell" onClick={() => toggleSort('totalRevenue')}>Revenue {sortIcon('totalRevenue', sortField, sortDir)}</th>
                    <th className="py-3.5 px-4 text-right cursor-pointer select-none hover:text-slate-700 hidden lg:table-cell" onClick={() => toggleSort('createdAt')}>Since {sortIcon('createdAt', sortField, sortDir)}</th>
                  ...
                  {paged.map(c => {
                    ...
                    return (
                      ...
                      <tr className="border-b border-slate-50 transition-colors hover:bg-slate-50 cursor-pointer" onClick={() => setExpandedId(expandedId === c.id ? null : c.id)}>
                        ...
                        <td className="py-3.5 px-4 text-sm text-slate-500 truncate max-w-[160px]">{c.email}</td>
                        <td className="py-3.5 px-4 text-sm text-slate-500 hidden md:table-cell">{c.city}</td>
                        <td className="py-3.5 px-4 hidden lg:table-cell">
                          <div className="flex flex-wrap gap-1">
                            {tags.length === 0 ? <span className="text-xs text-slate-400">—</span> : tags.slice(0, 3).map((t, i) => <span key={i} className={`inline-block rounded-full border px-2 py-0.5 text-[10px] font-medium ${TAG_COLORS[i % TAG_COLORS.length]}`}>{t}</span>)}
                            {tags.length > 3 && <span className="text-[10px] text-slate-400">+{tags.length - 3}</span>}
                          </div>
                        </td>
                        <td className="py-3.5 px-4 text-right text-sm font-semibold text-slate-900">{c.totalJobs}</td>
                        <td className="py-3.5 px-4 text-right text-sm font-semibold text-slate-900 hidden sm:table-cell">{fc(c.totalRevenue)}</td>
      ...
      {showModal && (
        <>
          <div className="fixed inset-0 z-50 bg-slate-900/40 flex items-start justify-center pt-[5vh] pb-8 px-4 overflow-y-auto" onClick={() => { setShowModal(false); resetForm(); }}>
            <div
              className={`w-full max-w-xl bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden ${shakeKey ? 'animate-[shake_0.4s_ease-in-out]' : ''}`}
              onClick={e => e.stopPropagation()}
            >
              {/* Header */}
              <div className="px-6 pt-6 pb-4 border-b border-slate-100">
                <h2 className="text-lg font-bold text-slate-900">{editingId ? 'Edit Client' : 'Add New Client'}</h2>
                <p className="text-sm text-slate-500 mt-0.5">{editingId ? 'Update client information.' : 'Fill in the details to add a new client.'}</p>
              </div>
              ...
              <div className="px-6 py-5 space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full h-11 px-4 bg-white border rounded-xl text-sm text-slate-900 placeholder:text-slate-400 outline-none transition-all ${errors.fName ? 'border-red-300 focus:ring-2 focus:ring-red-100' : 'border-slate-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-100'}`}
                    />
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <div className="relative shrink-0">
                        <select value={fCountry} onChange={e => { setFCountry(e.target.value); setFPhoneDisp(''); }} className="h-11 pl-3 pr-7 bg-white border border-slate-200 rounded-xl text-sm text-slate-700 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 appearance-none cursor-pointer">
                      ...
                      <input
                        ...
                        className="flex-1 h-11 px-4 bg-white border border-slate-200 rounded-xl text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all"
                      />
                    </div>
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    ...
                    <input type="text" placeholder="Austin" value={fCity} onChange={e => { setFCity(e.target.value); setTouched(t => ({...t, fCity: true})); }} className={`w-full h-11 px-4 bg-white border rounded-xl text-sm text-slate-900 placeholder:text-slate-400 outline-none transition-all ${errors.fCity ? 'border-red-300 focus:ring-2 focus:ring-red-100' : 'border-slate-200 focus:border-blue-400 focus:ring-2 focus:ring-blue-100'}`} />
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {fTags.map((tag, i) => (
                        <span key={i} className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-[11px] font-medium ${TAG_COLORS[i % TAG_COLORS.length]}`}>
                          {tag}
                          <button onClick={() => { const next = fTags.filter((_, idx) => idx !== i); setFTags(next); setFTagsInput(next.join(', ')); }} className="hover:text-slate-900 transition-colors">
                ...
                <div>
                  ...
                  <textarea rows={3} placeholder="Additional notes about this client..." value={fNotes} onChange={e => setFNotes(e.target.value)} className="w-full px-4 py-3 bg-white border border-slate-200 rounded-xl text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all resize-none" />
                </div>
              </div>

              {/* Sticky Footer */}
              <div className="sticky bottom-0 px-6 py-4 bg-white border-t border-slate-100 flex items-center justify-end gap-3">
                <button onClick={() => { setShowModal(false); resetForm(); }} className="h-10 px-5 rounded-xl border border-slate-200 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors">Cancel</button>
                <button onClick={handleSave} disabled={saving} className="h-10 px-5 rounded-xl bg-blue-500 text-white text-sm font-semibold hover:bg-blue-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm">
      ...
      {showDelete && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 flex items-center justify-center p-4" onClick={() => setShowDelete(null)}>
          <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl border border-slate-200 p-6" onClick={e => e.stopPropagation()}>
            <h3 className="text-base font-bold text-slate-900 mb-1">Delete Client</h3>
            <p className="text-sm text-slate-500 mb-5">Are you sure you want to delete this client? This action cannot be undone.</p>
            <div className="flex items-center justify-end gap-3">
              <button onClick={() => setShowDelete(null)} className="h-10 px-5 rounded-xl border border-slate-200 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors">Cancel</button>
