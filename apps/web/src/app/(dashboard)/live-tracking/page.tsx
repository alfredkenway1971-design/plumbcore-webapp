  ...
  return (
    <div className="p-4 sm:p-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Live Tracking</h1>
          <p className="text-sm text-slate-500 mt-0.5">Real-time technician locations and arrival notifications</p>
        </div>
          ...
          return (
            <div key={loc.techId} className="rounded-xl ring-1 ring-black/5 bg-white p-4 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-900">{loc.techName}</p>
                  <p className="text-xs text-slate-500">{getTechStatusLabel(loc.status)}</p>
                </div>
                <span className="ml-auto text-[10px] text-slate-400">🔋{loc.batteryLevel}%</span>
              </div>
              {currentJob && (
                <div className="rounded-xl bg-slate-50 p-2.5 text-sm">
                  <p className="font-medium text-slate-900">{currentJob.title}</p>
                  <p className="text-xs text-slate-500">{currentJob.clientName} — {getJobAddress(currentJob.id)}</p>
                </div>
              )}
              ...
              <p className="text-[10px] text-slate-400">Updated {formatTime(loc.updatedAt)}</p>
            </div>
          );
        })}
      </div>
      ...
      {notifs.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-slate-900 mb-2">Recent Notifications</h3>
          <div className="space-y-1.5">
            {notifs.slice(0, 10).map(n => (
              <div key={n.id} className="flex items-center gap-3 rounded-xl bg-slate-50 px-3.5 py-2.5 text-sm">
                <span>{n.type === 'en_route' ? '🚗' : n.type === 'ten_min' ? '⏱️' : '📍'}</span>
                <span className="text-slate-700"><strong>{n.techName}</strong> → <strong>{n.customerName}</strong></span>
                <span className="text-xs text-slate-400 ml-auto">{formatTime(n.sentAt)}</span>
              </div>
            ))}
          </div>
        </div>
