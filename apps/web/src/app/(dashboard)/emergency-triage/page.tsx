  ...
  return (
    <div className="space-y-4 animate-pulse">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="h-32 rounded-xl bg-white/5 border border-gray-200" />
      ))}
    </div>
  );
}
...
export default function EmergencyTriagePage() {
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <div className="h-8 w-64 rounded bg-gray-50 animate-pulse" />
        <CallCardsSkeleton />
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-gray-900">Emergency Call Triage</h1>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Emergency Call Triage</h1>
          <p className="text-sm text-gray-500 mt-1">Manage after-hours emergency calls and dispatch response.</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="flex h-3 w-3 rounded-full bg-red-500 animate-pulse" />
          <span className="text-sm font-semibold text-gray-900">{activeCalls} Active {activeCalls === 1 ? 'Call' : 'Calls'}</span>
        </div>
      </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-sm font-semibold text-gray-900">{call.callerName}</h3>
                    <a href={`tel:${call.phone}`} className="text-xs text-blue-600 hover:underline">{call.phone}</a>
                  ...
                  <p className="text-sm text-gray-500 mb-2">{call.issue}</p>
                ...
                <div className="flex sm:flex-col items-center sm:items-stretch gap-2 shrink-0">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-lg bg-white/5 border border-gray-200 px-2.5 py-1.5 text-xs font-medium text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-colors"
                    >
                      <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
      ...
      <Card variant="bordered" padding="md">
        ...
        <div className="space-y-3">
          <textarea
            ...
            rows={4}
            className="w-full rounded-lg border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 placeholder-gray-400 outline-none transition-all focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20 resize-none"
          />
          <div className="flex justify-between items-center">
            <p className="text-xs text-gray-500">{triageInput.length} characters</p>
            <Button
              variant="primary"
              size="sm"
        ...
        {triageLoading && (
          <div className="mt-4 space-y-3 animate-pulse">
            <div className="h-4 w-48 rounded bg-gray-50" />
        ...
        {triageResult && !triageLoading && (
          <div className="mt-4 rounded-xl bg-blue-50 border border-blue-200 p-4 space-y-3">
            <div className="flex items-start gap-3">
              <div className="mt-0.5 shrink-0 flex h-8 w-8 items-center justify-center rounded-full bg-blue-100 text-blue-600">
                <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" />
                </svg>
              </div>
              <div className="min-w-0 flex-1">
                <h3 className="text-sm font-semibold text-blue-600">AI Urgency Assessment</h3>
                <div className="mt-3 space-y-2">
                  <div className="flex items-center justify-between rounded-lg bg-white/80 px-3 py-2">
                    <span className="text-xs text-gray-500">Response Priority</span>
                    <span className="text-xs font-semibold text-gray-900">{triageResult.priority}</span>
                  </div>
                  <div className="flex items-center justify-between rounded-lg bg-white/80 px-3 py-2">
                    <span className="text-xs text-gray-500">Suggested Action</span>
                    <span className="text-xs text-gray-900 text-right max-w-[200px]">{triageResult.suggestedAction}</span>
                  </div>
                  <div className="flex items-center justify-between rounded-lg bg-white/80 px-3 py-2">
                    <span className="text-xs text-gray-500">Estimated Response Time</span>
                    <span className="text-xs font-semibold text-gray-900">{triageResult.estimatedResponseTime}</span>
                  </div>
                </div>
              </div>
            </div>
