...
export default function EmergencyTriagePage() {
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <div className="h-8 w-64 rounded bg-slate-50 animate-pulse" />
        <CallCardsSkeleton />
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-slate-900">Emergency Call Triage</h1>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Emergency Call Triage</h1>
          <p className="text-sm text-slate-500 mt-1">Manage after-hours emergency calls and dispatch response.</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="flex h-3 w-3 rounded-full bg-red-500 animate-pulse" />
          <span className="text-sm font-semibold text-slate-900">{activeCalls} Active {activeCalls === 1 ? 'Call' : 'Calls'}</span>
        </div>
      </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-sm font-semibold text-slate-900">{call.callerName}</h3>
                  ...
                  <p className="text-sm text-slate-500 mb-2">{call.issue}</p>
                ...
                <div className="flex sm:flex-col items-center sm:items-stretch gap-2 shrink-0">
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-xl bg-white/5 ring-1 ring-black/5 px-2.5 py-1.5 text-xs font-medium text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition-colors"
                    >
                      <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
      ...
      <Card variant="bordered" padding="md">
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center">
            <p className="text-xs text-slate-500">{triageInput.length} characters</p>
            <Button
              variant="primary"
              size="sm"
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-3 space-y-2">
                  <div className="flex items-center justify-between rounded-xl bg-white/80 px-3 py-2">
                    <span className="text-xs text-slate-500">Response Priority</span>
                    <span className="text-xs font-semibold text-slate-900">{triageResult.priority}</span>
                  </div>
                  <div className="flex items-center justify-between rounded-xl bg-white/80 px-3 py-2">
                    <span className="text-xs text-slate-500">Suggested Action</span>
                    <span className="text-xs text-slate-900 text-right max-w-[200px]">{triageResult.suggestedAction}</span>
                  </div>
                  <div className="flex items-center justify-between rounded-xl bg-white/80 px-3 py-2">
                    <span className="text-xs text-slate-500">Estimated Response Time</span>
                    <span className="text-xs font-semibold text-slate-900">{triageResult.estimatedResponseTime}</span>
                  </div>
                </div>
              </div>
            </div>
