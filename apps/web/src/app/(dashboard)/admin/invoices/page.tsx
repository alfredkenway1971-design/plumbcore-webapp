  ...
  return (
    <div className="max-w-[1440px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        {[ { label: 'Collected', value: '$' + totalRevenue.toLocaleString(), color: 'text-emerald-600', bg: 'bg-green-500/10' }, { label: 'Pending', value: '$' + pendingAmount.toLocaleString(), color: 'text-amber-600', bg: 'bg-amber-50' }, { label: 'Overdue', value: '$' + overdueAmount.toLocaleString(), color: 'text-red-600', bg: 'bg-red-50' } ].map((s, i) => (
      ...
      <div className="bg-white rounded-2xl ring-1 ring-white/5 shadow-sm ring-1 ring-black/5 overflow-hidden">
        <div className="px-5 py-4 flex flex-col sm:flex-row gap-3 border-b border-slate-100">
          <div className="relative flex-1">
            ...
            <input placeholder="Search invoices or companies..." value={search} onChange={e => setSearch(e.target.value)} className="w-full h-10 pl-10 pr-4 hover:bg-slate-50 border-0 rounded-xl text-sm text-white placeholder:text-slate-600 outline-none focus:ring-2 focus:ring-blue-500/20" />
          </div>
          <div className="flex gap-2">
            {['all', 'paid', 'pending', 'overdue', 'cancelled'].map(s => (
              <button key={s} onClick={() => setStatusFilter(s)} className={`h-10 px-3 rounded-xl text-xs font-semibold transition-all capitalize ${statusFilter === s ? 'bg-slate-700 text-slate-900' : 'hover:bg-slate-50 text-slate-400 hover:hover:bg-slate-50'}`}>{s}</button>
            ))}
          </div>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {filtered.slice(0, 25).map(inv => {
                ...
                return (
                  <tr key={inv.id} className="border-b border-slate-100 hover:hover:bg-slate-50 transition-colors">
