  ...
  return (
    <div className="max-w-[1440px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[ { label: 'Total Reviews', value: reviews.length, icon: MessageSquare, color: 'text-blue-500', bg: 'bg-blue-50' }, { label: 'Avg Rating', value: avgRating + ' ★', icon: Star, color: 'text-amber-500', bg: 'bg-amber-50' }, { label: 'Published', value: reviews.filter(r => r.status === 'published').length, icon: ThumbsUp, color: 'text-emerald-500', bg: 'bg-emerald-50' }, { label: 'Flagged', value: reviews.filter(r => r.status === 'flagged').length, icon: Filter, color: 'text-red-500', bg: 'bg-red-50' } ].map((s, i) => (
      ...
      <div className="bg-white rounded-2xl ring-1 ring-black/5 shadow-sm overflow-hidden">
        <div className="px-5 py-4 flex flex-col sm:flex-row gap-3 border-b border-slate-100">
          <div className="relative flex-1">
            ...
            <input placeholder="Search reviews..." value={search} onChange={e => setSearch(e.target.value)} className="w-full h-10 pl-10 pr-4 bg-slate-50 border-0 rounded-xl text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-blue-100" />
          </div>
            ...
            <div key={r.id} className="px-5 py-4 hover:bg-slate-50/50 transition-colors">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                  ...
                  <span className="text-xs text-slate-400">{r.helpful} found helpful</span>
                </div>
              </div>
            </div>
          ))}
