...
const reasonColors: Record<string, string> = {
  ...
  'Missing features': 'bg-amber-50 text-amber-700',
  'Poor support': 'bg-orange-50 text-orange-700',
  'No longer needed': 'bg-slate-100 text-slate-600',
  'Switched competitor': 'bg-purple-50 text-purple-700',
  'Budget cuts': 'bg-blue-50 text-blue-700',
};
  ...
  return (
    <div className="max-w-[1440px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[ { label: 'Total Churned', value: churnedData.length, icon: XCircle, color: 'text-red-500', bg: 'bg-red-50' }, { label: 'MRR Lost', value: '$' + totalLostMrr.toLocaleString(), icon: ArrowDown, color: 'text-red-500', bg: 'bg-red-50' }, { label: 'Churn Rate', value: '2.1%', icon: TrendingDown, color: 'text-amber-500', bg: 'bg-amber-50' }, { label: 'Avg Lifetime', value: '14 mo', icon: Building2, color: 'text-blue-500', bg: 'bg-blue-50' } ].map((s, i) => (
      ...
      <div className="bg-white rounded-2xl ring-1 ring-black/5 shadow-sm overflow-hidden">
        <div className="px-5 py-4 flex flex-col sm:flex-row gap-3 border-b border-slate-100">
          <div className="relative flex-1">
            ...
            <input placeholder="Search accounts..." value={search} onChange={e => setSearch(e.target.value)} className="w-full h-10 pl-10 pr-4 bg-slate-50 border-0 rounded-xl text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-blue-100" />
          </div>
          <div className="flex gap-2 flex-wrap">
            <button onClick={() => setReasonFilter('all')} className={`h-10 px-3 rounded-xl text-xs font-semibold transition-all ${reasonFilter === 'all' ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>All</button>
            {churnReasons.map(r => (
              <button key={r} onClick={() => setReasonFilter(r)} className={`h-10 px-3 rounded-xl text-xs font-semibold transition-all ${reasonFilter === r ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>{r}</button>
            ))}
          </div>
        </div>
                ...
                <tr key={c.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  <td className="py-3.5 px-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div><p className="text-sm font-semibold text-slate-900">{c.name}</p><p className="text-xs text-slate-400">{c.city}, {c.state}</p></div>
                    </div>
                  </td>
                  ...
                  <td className="py-3.5 px-5"><span className="text-sm text-slate-600">{c.churnDate}</span></td>
                  <td className="py-3.5 px-5"><span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-semibold ${reasonColors[c.churnReason] || 'bg-slate-50 text-slate-600'}`}>{c.churnReason}</span></td>
