      ...
      {editingPlan && (
        <Modal open={true} onClose={() => setEditingPlan(null)} title={`Edit ${editingPlan.name} Plan`}>
          <div className="space-y-4 p-4">
            <div><label className="text-xs font-medium text-slate-500">Plan Name</label><Input value={editForm.name} onChange={(e: any) => setEditForm(f => ({...f, name: e.target.value}))} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs font-medium text-slate-500">Price ($/mo)</label><Input type="number" value={editForm.price} onChange={(e: any) => setEditForm(f => ({...f, price: Number(e.target.value)}))} /></div>
