...
export default function PricingPlansPage() {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5">
      <div><h1 className="text-xl sm:text-2xl font-bold text-gray-900">Pricing Plans</h1><p className="text-sm text-gray-500 mt-0.5">Manage subscription plans, features, and pricing</p></div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {plans.filter(p => p.active).map(plan => (
          <Card key={plan.id} variant="bordered" padding="lg" className="flex flex-col">
            ...
            <p className="text-xs text-gray-500 mt-1">{plan.techLimit} techs • {plan.hoursLimit}hrs AI</p>
            <ul className="mt-4 space-y-1.5 flex-1">
              {plan.features.map((f, i) => <li key={i} className="text-xs text-gray-600 flex items-start gap-1.5"><span className="text-green-500">✓</span>{f}</li>)}
            </ul>
      ...
      {editingPlan && (
        <Modal open={true} onClose={() => setEditingPlan(null)} title={`Edit ${editingPlan.name} Plan`}>
          <div className="space-y-4 p-4">
            <div><label className="text-xs font-medium text-gray-500">Plan Name</label><Input value={editForm.name} onChange={(e: any) => setEditForm(f => ({...f, name: e.target.value}))} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label className="text-xs font-medium text-gray-500">Price ($/mo)</label><Input type="number" value={editForm.price} onChange={(e: any) => setEditForm(f => ({...f, price: Number(e.target.value)}))} /></div>
              <div><label className="text-xs font-medium text-gray-500">Tech Limit</label><Input type="number" value={editForm.techLimit} onChange={(e: any) => setEditForm(f => ({...f, techLimit: Number(e.target.value)}))} /></div>
            </div>
            <div><label className="text-xs font-medium text-gray-500">AI Hours Limit</label><Input type="number" value={editForm.hoursLimit} onChange={(e: any) => setEditForm(f => ({...f, hoursLimit: Number(e.target.value)}))} /></div>
            <div><label className="text-xs font-medium text-gray-500">Features (one per line)</label><textarea value={editForm.features} onChange={(e: any) => setEditForm(f => ({...f, features: e.target.value}))} rows={6} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm outline-none focus:border-blue-400" /></div>
