...
const trendColors: Record<string, string> = { up: 'text-emerald-500', down: 'text-red-500', flat: 'text-slate-400' };
  ...
  return (
    <div className="max-w-[1440px] mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="lg:col-span-2 bg-white rounded-2xl ring-1 ring-black/5 p-6 shadow-sm">
        ...
        <div className="bg-white rounded-2xl ring-1 ring-black/5 p-6 shadow-sm">
              ...
              return (
                <div key={f.featureKey}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <div className="flex items-center gap-1"><TrendIcon className={`w-3.5 h-3.5 ${trendColors[f.trend]}`} /><span className="text-xs font-semibold text-slate-600">{f.adoptionRate}%</span></div>
                  </div>
                  <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-blue-500 to-cyan-500 rounded-full" style={{width: f.adoptionRate + '%'}} />
                  </div>
                  <p className="text-xs text-slate-400 mt-1">{f.weeklyActiveUsers.toLocaleString()} weekly users</p>
                </div>
              );
            })}
          </div>
        ...
        <div className="px-5 py-4 border-b border-slate-100">
          <div className="flex items-center justify-between">
            ...
            <div className="relative w-64">
              ...
              <input placeholder="Search features..." value={search} onChange={e => setSearch(e.target.value)} className="w-full h-10 pl-10 pr-4 bg-slate-50 border-0 rounded-xl text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-blue-100" />
            </div>
          </div>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {filtered.map(f => {
                const TrendIcon = trendIcons[f.trend];
                return (
                  <tr key={f.featureKey} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    <td className="py-3.5 px-5"><span className="text-sm font-semibold text-slate-900">{f.featureName}</span></td>
                    <td className="py-3.5 px-5"><span className="text-sm text-slate-600">{f.totalEnabled.toLocaleString()}</span></td>
                    <td className="py-3.5 px-5"><span className="text-sm text-slate-600">{f.weeklyActiveUsers.toLocaleString()}</span></td>
                    <td className="py-3.5 px-5">
                      <div className="flex items-center gap-2"><div className="w-20 h-2 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-blue-500 rounded-full" style={{width: f.adoptionRate + '%'}} /></div><span className="text-xs font-semibold text-slate-600">{f.adoptionRate}%</span></div>
                    </td>
