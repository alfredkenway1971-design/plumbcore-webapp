...
const statusStyles: Record<number, { bg: string; text: string; icon: any }> = {
  200: { bg: 'bg-emerald-500/10', text: 'text-emerald-300', icon: CheckCircle },
  201: { bg: 'bg-emerald-500/10', text: 'text-emerald-300', icon: CheckCircle },
  400: { bg: 'bg-amber-500/10', text: 'text-amber-300', icon: AlertTriangle },
  401: { bg: 'bg-red-500/10', text: 'text-red-300', icon: XCircle },
  403: { bg: 'bg-red-500/10', text: 'text-red-300', icon: XCircle },
  404: { bg: 'bg-amber-500/10', text: 'text-amber-300', icon: AlertTriangle },
  500: { bg: 'bg-red-500/10', text: 'text-red-300', icon: XCircle },
};
  ...
  return (
    <div className="max-w-[1440px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-white">API Logs</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
        {[ { label: 'Total Requests', value: totalRequests, color: 'text-blue-500', bg: 'bg-blue-500/10' }, { label: 'Error Rate', value: errorRate + '%', color: errorRate > 5 ? 'text-red-500' : 'text-emerald-500', bg: errorRate > 5 ? 'bg-red-500/10' : 'bg-emerald-500/10' }, { label: 'Avg Latency', value: avgLatency + 'ms', color: 'text-purple-500', bg: 'bg-purple-500/10' } ].map((s, i) => (
      ...
      <div className="bg-[#0F172A] rounded-2xl ring-1 ring-white/5 shadow-lg shadow-black/30 overflow-hidden">
        <div className="px-5 py-4 flex flex-col sm:flex-row gap-3 border-b border-white/5">
          <div className="relative flex-1">
            ...
            <input placeholder="Search endpoints or IPs..." value={search} onChange={e => setSearch(e.target.value)} className="w-full h-10 pl-10 pr-4 bg-white/[0.02] border-0 rounded-xl text-sm text-white placeholder:text-slate-600 outline-none focus:ring-2 focus:ring-blue-500/20" />
          </div>
          <div className="flex gap-2">
            {['all', 'GET', 'POST', 'DELETE'].map(m => (
              <button key={m} onClick={() => setMethodFilter(m)} className={`h-10 px-3 rounded-xl text-xs font-semibold transition-all ${methodFilter === m ? 'bg-white/10 text-white' : 'bg-white/[0.02] text-slate-400 hover:bg-white/[0.04]'}`}>{m === 'all' ? 'All' : m}</button>
            ))}
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {filtered.map(l => {
                const s = statusStyles[l.status] || { bg: 'bg-white/[0.02]', text: 'text-slate-400', icon: Clock };
                const StatusIcon = s.icon;
                return (
                  <tr key={l.id} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors font-mono text-xs">
                    <td className="py-3 px-5 text-slate-500">{new Date(l.timestamp).toLocaleTimeString()}</td>
                    <td className="py-3 px-5"><span className={`font-semibold ${l.method === 'GET' ? 'text-blue-400' : l.method === 'POST' ? 'text-emerald-400' : 'text-red-400'}`}>{l.method}</span></td>
                    <td className="py-3 px-5"><span className="text-slate-300">{l.endpoint}</span></td>
                    ...
                    <td className="py-3 px-5"><span className={`${l.latency > 1000 ? 'text-red-400' : l.latency > 500 ? 'text-amber-400' : 'text-slate-500'}`}>{l.latency}ms</span></td>
                    <td className="py-3 px-5 text-slate-600">{l.ip}</td>
                  </tr>
                );
              })}
            </tbody>
