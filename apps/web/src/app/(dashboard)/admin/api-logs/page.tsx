...
const statusStyles: Record<number, { bg: string; text: string; icon: any }> = {
  ...
  400: { bg: 'bg-amber-50', text: 'text-amber-700', icon: AlertTriangle },
  401: { bg: 'bg-red-50', text: 'text-red-700', icon: XCircle },
  403: { bg: 'bg-red-50', text: 'text-red-700', icon: XCircle },
  404: { bg: 'bg-amber-50', text: 'text-amber-700', icon: AlertTriangle },
  ...
  return (
    <div className="max-w-[1440px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-slate-900">API Logs</h1>
      ...
      <div className="bg-white rounded-2xl ring-1 ring-black/5 shadow-sm overflow-hidden">
        <div className="px-5 py-4 flex flex-col sm:flex-row gap-3 border-b border-slate-100">
          <div className="relative flex-1">
            ...
            <input placeholder="Search endpoints or IPs..." value={search} onChange={e => setSearch(e.target.value)} className="w-full h-10 pl-10 pr-4 bg-slate-50 border-0 rounded-xl text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-blue-100" />
          </div>
          <div className="flex gap-2">
            {['all', 'GET', 'POST', 'DELETE'].map(m => (
              <button key={m} onClick={() => setMethodFilter(m)} className={`h-10 px-3 rounded-xl text-xs font-semibold transition-all ${methodFilter === m ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>{m === 'all' ? 'All' : m}</button>
            ))}
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {filtered.map(l => {
                const s = statusStyles[l.status] || { bg: 'bg-slate-50', text: 'text-slate-600', icon: Clock };
                const StatusIcon = s.icon;
                return (
                  <tr key={l.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors font-mono text-xs">
                    ...
                    <td className="py-3 px-5"><span className={`${l.latency > 1000 ? 'text-red-600' : l.latency > 500 ? 'text-amber-600' : 'text-slate-500'}`}>{l.latency}ms</span></td>
