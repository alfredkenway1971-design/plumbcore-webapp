  ...
  return (
    <div className="max-w-[1440px] mx-auto">
                    ...
                    <div key={field.key} className="flex items-center justify-between gap-4">
                      ...
                      <div className="shrink-0">
                        {field.type === 'toggle' ? (
                          ...
                          <select value={value} onChange={e => update(field.key, e.target.value)} className="h-9 px-3 hover:bg-muted border-0 rounded-lg text-sm text-foreground outline-none focus:ring-2 focus:ring-primary/20">
                            {field.options?.map(o => <option key={o} value={o}>{o.toUpperCase()}</option>)}
                          </select>
                        ) : (
                          <input type={field.type} value={value} onChange={e => update(field.key, field.type === 'number' ? parseInt(e.target.value) || 0 : e.target.value)} className="h-9 w-48 px-3 hover:bg-muted border-0 rounded-lg text-sm text-foreground outline-none focus:ring-2 focus:ring-primary/20 text-right" />
                        )}
                      </div>
                    </div>
                  );
      ...
      <div className="flex flex-col items-center justify-center pt-8 pb-4">
        <button
          onClick={handleSave}
          className="inline-flex items-center justify-center gap-3 h-12 px-10 rounded-xl bg-primary text-white text-base font-semibold hover:bg-primary/90 transition-all shadow-md ring-1 ring-black/10 min-w-[200px]"
        >
