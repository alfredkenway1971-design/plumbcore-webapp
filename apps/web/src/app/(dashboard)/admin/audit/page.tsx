    ...
    <div className="max-w-[1440px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Track all platform activity and changes</p>
        </div>
      ...
      <div className="bg-white rounded-2xl ring-1 ring-slate-200 shadow-sm ring-1 ring-black/5 overflow-hidden">
        <div className="px-5 py-4 flex flex-col sm:flex-row gap-3 border-b border-border/50">
          <div className="relative flex-1">
            ...
            <input placeholder="Search events..." value={search} onChange={e => setSearch(e.target.value)} className="w-full h-10 pl-10 pr-4 hover:bg-muted border-0 rounded-xl text-sm text-foreground placeholder:text-muted-foreground outline-none focus:ring-2 focus:ring-primary/20" />
          </div>
          <div className="flex gap-2">
            {['all', 'success', 'warning', 'error', 'info'].map((s: any) => (
              <button key={s} onClick={() => setSeverityFilter(s)} className={`h-10 px-3 rounded-xl text-xs font-semibold transition-all capitalize ${severityFilter === s ? 'bg-primary text-white' : 'hover:bg-muted text-muted-foreground'}`}>{s}</button>
            ))}
          </div>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {filtered.map(a => {
                ...
                return (
                  <tr key={a.id} className="border-b border-border/50 hover:bg-muted transition-colors">
                    <td className="py-3.5 px-5"><span className="text-xs text-muted-foreground font-mono">{new Date(a.timestamp).toLocaleString()}</span></td>
                    <td className="py-3.5 px-5">
                      <div className="flex items-center gap-2"><Building2 className="w-4 h-4 text-muted-foreground" /><span className="text-sm font-medium text-foreground">{a.companyName}</span></div>
                    </td>
                    <td className="py-3.5 px-5"><span className="text-sm text-muted-foreground/80">{typeLabels[a.type] || a.type}</span></td>
                    <td className="py-3.5 px-5"><span className="text-sm text-muted-foreground/80">{a.description}</span></td>
