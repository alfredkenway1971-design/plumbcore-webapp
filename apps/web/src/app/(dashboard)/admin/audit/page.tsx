...
const severityStyles: Record<string, { bg: string; text: string; icon: any; dot: string }> = {
  ...
  info: { bg: 'bg-blue-50', text: 'text-blue-700', icon: Info, dot: 'bg-blue-500' },
};
  ...
  return (
    <div className="max-w-[1440px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[ { label: 'Total Events', value: recentActivity.length, color: 'text-blue-500', bg: 'bg-blue-50' }, { label: 'Success', value: recentActivity.filter(a => a.severity === 'success').length, color: 'text-emerald-500', bg: 'bg-emerald-50' }, { label: 'Warnings', value: recentActivity.filter(a => a.severity === 'warning').length, color: 'text-amber-500', bg: 'bg-amber-50' }, { label: 'Errors', value: recentActivity.filter(a => a.severity === 'error').length, color: 'text-red-500', bg: 'bg-red-50' } ].map((s, i) => (
      ...
      <div className="bg-white rounded-2xl ring-1 ring-black/5 shadow-sm overflow-hidden">
        <div className="px-5 py-4 flex flex-col sm:flex-row gap-3 border-b border-slate-100">
          <div className="relative flex-1">
            ...
            <input placeholder="Search events..." value={search} onChange={e => setSearch(e.target.value)} className="w-full h-10 pl-10 pr-4 bg-slate-50 border-0 rounded-xl text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-blue-100" />
          </div>
          <div className="flex gap-2">
            {['all', 'success', 'warning', 'error', 'info'].map(s => (
              <button key={s} onClick={() => setSeverityFilter(s)} className={`h-10 px-3 rounded-xl text-xs font-semibold transition-all capitalize ${severityFilter === s ? 'bg-slate-900 text-white' : 'bg-slate-50 text-slate-600 hover:bg-slate-100'}`}>{s}</button>
            ))}
          </div>
        </div>
                ...
                return (
                  <tr key={a.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-3.5 px-5">
                      <div className="flex items-center gap-2"><Building2 className="w-4 h-4 text-slate-400" /><span className="text-sm font-medium text-slate-700">{a.companyName}</span></div>
                    </td>
                    <td className="py-3.5 px-5"><span className="text-sm text-slate-600">{typeLabels[a.type] || a.type}</span></td>
                    <td className="py-3.5 px-5"><span className="text-sm text-slate-600">{a.description}</span></td>
