...
const severityStyles: Record<string, { bg: string; text: string; icon: any; dot: string }> = {
  success: { bg: 'bg-emerald-500/10', text: 'text-emerald-300', icon: CheckCircle, dot: 'bg-emerald-500' },
  warning: { bg: 'bg-amber-500/10', text: 'text-amber-300', icon: AlertTriangle, dot: 'bg-amber-500' },
  error: { bg: 'bg-red-500/10', text: 'text-red-300', icon: XCircle, dot: 'bg-red-500' },
  info: { bg: 'bg-blue-500/10', text: 'text-blue-300', icon: Info, dot: 'bg-blue-500' },
};
  ...
  return (
    <div className="max-w-[1440px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[ { label: 'Total Events', value: recentActivity.length, color: 'text-blue-500', bg: 'bg-blue-500/10' }, { label: 'Success', value: recentActivity.filter(a => a.severity === 'success').length, color: 'text-emerald-500', bg: 'bg-emerald-500/10' }, { label: 'Warnings', value: recentActivity.filter(a => a.severity === 'warning').length, color: 'text-amber-400', bg: 'bg-amber-500/10' }, { label: 'Errors', value: recentActivity.filter(a => a.severity === 'error').length, color: 'text-red-500', bg: 'bg-red-500/10' } ].map((s, i) => (
      ...
      <div className="bg-[#0F172A] rounded-2xl ring-1 ring-white/5 shadow-lg shadow-black/30 overflow-hidden">
        <div className="px-5 py-4 flex flex-col sm:flex-row gap-3 border-b border-white/5">
          <div className="relative flex-1">
            ...
            <input placeholder="Search events..." value={search} onChange={e => setSearch(e.target.value)} className="w-full h-10 pl-10 pr-4 bg-white/[0.02] border-0 rounded-xl text-sm text-white placeholder:text-slate-600 outline-none focus:ring-2 focus:ring-blue-500/20" />
          </div>
          <div className="flex gap-2">
            {['all', 'success', 'warning', 'error', 'info'].map(s => (
              <button key={s} onClick={() => setSeverityFilter(s)} className={`h-10 px-3 rounded-xl text-xs font-semibold transition-all capitalize ${severityFilter === s ? 'bg-white/10 text-white' : 'bg-white/[0.02] text-slate-400 hover:bg-white/[0.04]'}`}>{s}</button>
            ))}
          </div>
        </div>
                ...
                return (
                  <tr key={a.id} className="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="py-3.5 px-5">
                      <div className="flex items-center gap-2"><Building2 className="w-4 h-4 text-slate-600" /><span className="text-sm font-medium text-slate-300">{a.companyName}</span></div>
                    </td>
