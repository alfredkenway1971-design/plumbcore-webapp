  ...
  return (
    <div className="max-w-[1440px] mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[ { label: 'At-Risk Accounts', value: atRiskAccounts.length, icon: AlertTriangle, color: 'text-red-500', bg: 'bg-red-50' }, { label: 'MRR at Risk', value: '$' + totalMrrAtRisk.toLocaleString(), icon: ArrowDown, color: 'text-amber-500', bg: 'bg-amber-50' }, { label: 'Avg Churn Probability', value: avgChurnProb + '%', icon: AlertTriangle, color: 'text-orange-500', bg: 'bg-orange-50' }, { label: 'Reps Assigned', value: new Set(atRiskAccounts.map(a => a.assignedRep)).size, icon: Phone, color: 'text-blue-500', bg: 'bg-blue-50' } ].map((s, i) => (
                ...
                <tr key={a.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  <td className="py-3.5 px-5">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl bg-red-50 flex items-center justify-center"><Building2 className="w-4 h-4 text-red-500" /></div>
                  ...
                  <td className="py-3.5 px-5"><span className="text-sm text-slate-600">{a.techCount}</span></td>
                  <td className="py-3.5 px-5">
                    <div className="flex items-center gap-2"><div className="w-20 h-2 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full rounded-full ${a.churnProbability >= 0.7 ? 'bg-red-500' : a.churnProbability >= 0.4 ? 'bg-amber-500' : 'bg-emerald-500'}`} style={{width: a.churnProbability * 100 + '%'}} /></div><span className="text-xs font-bold text-slate-700">{Math.round(a.churnProbability * 100)}%</span></div>
                  </td>
                  <td className="py-3.5 px-5"><span className="text-sm text-slate-600">{a.reason}</span></td>
                  ...
                  <td className="py-3.5 px-5">
                    <div className="flex gap-2">
                      ...
                      <button className="h-8 px-3 rounded-lg ring-1 ring-black/5 text-slate-600 text-xs font-medium hover:bg-slate-50 transition-colors">View</button>
                    </div>
                  </td>
                </tr>
              ))}
