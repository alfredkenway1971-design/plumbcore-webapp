...
function StatCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm transition-all hover:shadow-[0_4px_12px_rgba(0,0,0,0.06)]">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          <h2 className="text-lg font-semibold text-slate-900">Create Maintenance Plan</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Visit Interval</label>
              <select
                value={intervalMonths}
                onChange={(e) => setIntervalMonths(e.target.value)}
                className="w-full h-10 px-3 rounded-xl border border-slate-200 bg-white text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-300"
              >
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full h-20 px-3 py-2 rounded-xl border border-slate-200 bg-white text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-300 resize-none"
            />
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Included Services (one per line)</label>
            <textarea
              value={services}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Complete Visit</h2>
            <p className="text-sm text-slate-500">{visit.customerName} — {visit.planName}</p>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 text-xl leading-none">&times;</button>
        </div>
        <div className="p-5 space-y-4">
          <div>
            ...
            <textarea
              ...
              className="w-full h-24 px-3 py-2 rounded-xl border border-slate-200 bg-white text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-300 resize-none"
            />
          </div>

          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-medium text-slate-700">Upsell Opportunities</label>
              {upsells.length > 0 && (
              ...
              <div className="space-y-2 mb-3">
                {upsells.map((u, i) => (
                  <div key={i} className="flex items-center justify-between bg-slate-50 rounded-xl px-3 py-2 text-sm">
                    <span className="text-slate-700 flex-1">{u.description}</span>
  ...
  if (!hasAccess) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
        ...
        <h2 className="text-xl font-semibold text-slate-900 mb-2">Upgrade to Access</h2>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Maintenance Plans</h1>
          <p className="text-sm text-slate-500 mt-1">Create recurring maintenance contracts and manage subscribers</p>
        </div>
              ...
              <Card key={plan.id} className="p-5 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-slate-900 mt-1">{formatCurrency(plan.price_monthly)}<span className="text-sm font-normal text-slate-400">/mo</span></p>
                  </div>
                ...
                <div className="text-xs text-slate-400 mb-3">
                  Visit interval: <span className="font-medium text-slate-600">Every {plan.interval_months} month{plan.interval_months !== 1 ? 's' : ''}</span>
                </div>
                {plan.benefits.length > 0 && (
                  <div className="mb-3">
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Benefits</p>
                ...
                {plan.included_services.length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Included Services</p>
                    <div className="flex flex-wrap gap-1.5">
                      {plan.included_services.map((s, i) => (
                        <span key={i} className="text-[11px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded-full">{s}</span>
                      ))}
                    </div>
                  </div>
                )}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100">
                    ...
                    <th className="text-left text-xs font-semibold text-slate-400 uppercase tracking-wider px-4 py-3">Plan</th>
                    <th className="text-left text-xs font-semibold text-slate-400 uppercase tracking-wider px-4 py-3">Status</th>
                    <th className="text-left text-xs font-semibold text-slate-400 uppercase tracking-wider px-4 py-3 hidden sm:table-cell">Started</th>
                    <th className="text-left text-xs font-semibold text-slate-400 uppercase tracking-wider px-4 py-3 hidden md:table-cell">Next Billing</th>
                    <th className="text-left text-xs font-semibold text-slate-400 uppercase tracking-wider px-4 py-3 hidden lg:table-cell">Next Visit</th>
                    <th className="text-right text-xs font-semibold text-slate-400 uppercase tracking-wider px-5 py-3">Auto-Renew</th>
                  </tr>
                </thead>
                <tbody>
                  {subscribers.map((sub) => {
                    ...
                    return (
                      <tr key={sub.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-4 py-3.5 hidden sm:table-cell text-slate-500">{formatDate(sub.start_date)}</td>
                        <td className="px-4 py-3.5 hidden md:table-cell">
                          <div className="flex items-center gap-1.5">
                            <span className="text-slate-600">{formatDate(sub.next_billing_date)}</span>
                        ...
                        <td className="px-5 py-3.5 text-right">
                          <span className={`text-xs font-semibold ${sub.auto_renew ? 'text-emerald-600' : 'text-slate-400'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {upcomingVisits.length === 0 ? (
            ...
            upcomingVisits.map((visit) => {
              ...
              return (
                <div key={visit.id} className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <h3 className="font-semibold text-slate-900">{visit.customerName}</h3>
                      <p className="text-sm text-slate-500">{visit.planName}</p>
                    </div>
                  ...
                  <div className="flex items-center gap-3 text-sm text-slate-600 mb-3">
                    ...
                    {days >= 0 && (
                      <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-full ${
                        days <= 7 ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'
                      }`}>
                  ...
                  {visit.notes && (
                    <p className="text-sm text-slate-500 mb-3 bg-slate-50 rounded-xl px-3 py-2">{visit.notes}</p>
                  )}
