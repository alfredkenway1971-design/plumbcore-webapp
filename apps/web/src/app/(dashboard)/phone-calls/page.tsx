    ...
    <div className="p-4 sm:p-6 space-y-5">
      ...
      <div className="flex gap-2">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-8 w-28 rounded-lg bg-gray-50 animate-pulse" />
        ))}
      </div>
...
export default function PhoneCallsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-5">
      {/* Header */}
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Phone Calls</h1>
        <p className="text-sm text-gray-500 mt-0.5">Manage calls and Twilio integration</p>
      </div>

      {/* Tabs */}
      <div className="flex items-center gap-1 rounded-lg bg-gray-50/50 p-1 w-fit">
        {tabs.map((tab) => (
          <button
            key={tab.key}
      ...
      {activeTab === 'active' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <h2 className="text-sm font-semibold text-gray-700">Current Call</h2>
            {currentCall ? (
              <Card variant="bordered" padding="lg">
                <div className="flex flex-col items-center text-center space-y-4">
                  ...
                  <div>
                    <p className="text-lg font-semibold text-gray-900">{currentCall.caller}</p>
                    <p className="text-sm text-gray-500">{currentCall.phoneNumber}</p>
                  </div>
                  <StatusBadge status={currentCall.status} size="md" />
                  <p className="text-sm text-gray-400">{currentCall.duration}</p>
          ...
          {/* Dial Pad */}
          <div className="space-y-4">
            <h2 className="text-sm font-semibold text-gray-700">Dial Pad</h2>
                ...
                <div className="grid grid-cols-3 gap-2 w-full max-w-[240px]">
                  {['1', '2', '3', '4', '5', '6', '7', '8', '9', '*', '0', '#'].map(digit => (
                    <button
                      ...
                      className="flex h-14 w-full items-center justify-center rounded-xl bg-gray-100 text-lg font-semibold text-gray-700 hover:bg-gray-200 active:bg-gray-300 transition-colors"
                    >
                      {digit}
                    </button>
                  ))}
              ...
              <table className="w-full min-w-[700px]">
                <thead>
                  <tr className="border-b border-gray-200">
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Caller</th>
                    <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-gray-500">Duration</th>
                    <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-gray-500">Direction</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredCallLog.map((entry) => (
                    <tr key={entry.id} className="transition-colors hover:bg-gray-50">
                      <td className="px-4 py-3 text-sm text-gray-500 whitespace-nowrap">{entry.dateTime}</td>
                      <td className="px-4 py-3 text-sm font-medium text-gray-900 whitespace-nowrap">{entry.caller}</td>
                      <td className="px-4 py-3 text-sm text-gray-500 text-center whitespace-nowrap">{entry.duration}</td>
      ...
      {activeTab === 'settings' && (
        <div className="max-w-2xl space-y-5">
          {/* Twilio Connection */}
          <Card variant="bordered" padding="lg">
            <h3 className="text-base font-semibold text-gray-900 mb-4">Twilio Integration</h3>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-700">Twilio Phone Number</p>
          ...
          {/* Call Settings */}
          <Card variant="bordered" padding="lg">
            <h3 className="text-base font-semibold text-gray-900 mb-4">Call Settings</h3>
            <div className="space-y-4">
              {/* Call Recording Toggle */}
              <label className="flex items-center justify-between cursor-pointer">
                <div>
                  <p className="text-sm font-medium text-gray-700">Call Recording</p>
                  <p className="text-xs text-gray-400">Record all inbound and outbound calls</p>
                </div>
                <button
