...
function ReceptionistSkeleton() {
  return (
    <div className="space-y-5 animate-pulse">
      <div className="h-8 w-64 rounded bg-gray-50" />
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-gray-900">AI Voice Receptionist</h1>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">AI Voice Receptionist</h1>
          <p className="text-sm text-gray-500 mt-1">Manage your AI receptionist settings and view call history.</p>
        </div>
      </div>
      ...
      <Card variant="bordered" padding="md">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${isActive ? 'bg-green-100' : 'bg-gray-100'}`}>
              <svg className={`h-5 w-5 ${isActive ? 'text-green-600' : 'text-gray-400'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" />
              </svg>
            </div>
            <div>
              <p className="text-sm font-semibold text-gray-900">Receptionist is {isActive ? 'Active' : 'Paused'}</p>
              <p className="text-xs text-gray-500">{isActive ? 'Taking calls and booking appointments' : 'Calls are not being answered by AI'}</p>
            </div>
          </div>
          <button
      ...
      {/* Call History Table */}
      <Card variant="bordered" padding="md">
        <h2 className="text-sm font-semibold text-gray-900 mb-4">Call History</h2>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Date/Time</th>
                  <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Caller</th>
                  <th className="px-3 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-gray-500">Duration</th>
                  <th className="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Issue Summary</th>
                  <th className="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wider text-gray-500">Outcome</th>
                  <th className="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wider text-gray-500">Actions</th>
                </tr>
              </thead>
              <tbody>
                {callHistory.map((entry) => (
                  <tr key={entry.id} className="border-b border-gray-200 transition-colors hover:bg-white/[0.02]">
                    <td className="px-3 py-3 text-gray-900 text-xs whitespace-nowrap">{entry.dateTime}</td>
                    <td className="px-3 py-3 text-gray-900 font-medium whitespace-nowrap">{entry.caller}</td>
                    <td className="px-3 py-3 text-right text-gray-500 whitespace-nowrap">{entry.duration}</td>
                    <td className="px-3 py-3 text-gray-500 max-w-[250px] truncate">{entry.issueSummary}</td>
                    ...
                    <td className="px-3 py-3 text-center">
                      <button className="rounded-lg bg-white/5 border border-gray-200 px-2 py-1 text-[10px] font-medium text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-colors">
                        View Details
                      </button>
                    </td>
                  </tr>
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 outline-none transition-all focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20 resize-none"
            />
          </div>

          {/* After-Hours Message */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">After-Hours Message</label>
            <textarea
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Booking Start Time</label>
              <input
                type="time"
                value={settings.bookingHoursStart}
                onChange={(e) => setSettings(prev => ({ ...prev, bookingHoursStart: e.target.value }))}
                className="w-full rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-900 outline-none transition-all focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/20"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">Booking End Time</label>
              <input
                type="time"
          ...
          {/* Transfer Condition */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Transfer to Human If</label>
            <select
