...
export default function InvoicePrintPage() {
  ...
  return (
    <>
      ...
      <div className="no-print sticky top-0 z-10 border-b border-border bg-white px-6 py-3 flex items-center justify-between">
        <button
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-muted-foreground/80">Printable invoice view</span>
          <button
      ...
      <div className="max-w-4xl mx-auto p-8" style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}>
        ...
        <div className="flex justify-between items-start mb-12">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{companyCity}</p>
          ...
          <div className="text-right">
            ...
            <p className="text-lg font-semibold text-foreground mt-1">{invoice.id}</p>
          </div>
        </div>
          ...
          <div className="text-right">
            <div className="space-y-1.5">
              ...
              <div className="flex justify-end gap-4 text-sm">
                ...
                <span className="font-medium text-foreground">{invoice.jobTitle} ({invoice.jobId})</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <table className="w-full min-w-[500px] sm:min-w-0">
          <thead>
            <tr className="border-b-2 border-gray-300">
              <th className="py-3 pr-4 text-left text-xs font-semibold uppercase tracking-wider text-muted-foreground">Description</th>
              <th className="py-3 px-4 text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground">Qty</th>
              <th className="py-3 px-4 text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground">Unit Price</th>
              <th className="py-3 pl-4 text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground">Total</th>
            </tr>
          </thead>
          <tbody>
            {invoice.lineItems.map((item: any, idx: number) => (
              <tr key={idx} className="border-b border-border">
                <td className="py-4 pr-4 text-sm text-foreground">{item.description}</td>
                <td className="py-4 px-4 text-sm text-foreground text-right">{item.quantity}</td>
                <td className="py-4 px-4 text-sm text-foreground text-right">{formatCurrency(item.unitPrice)}</td>
                <td className="py-4 pl-4 text-sm font-medium text-foreground text-right">{formatCurrency(item.total)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="border-t border-border pt-6">
          <h4 className="text-sm font-semibold text-foreground mb-1">Payment Terms</h4>
