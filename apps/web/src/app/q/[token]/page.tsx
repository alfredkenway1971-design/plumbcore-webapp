    ...
    <div className="flex items-center gap-1">
      {steps.map((s, i) => (
        <div key={s} className="flex-1 flex flex-col items-center">
          <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${i <= idx ? 'bg-primary text-white' : 'bg-muted text-muted-foreground/80'}`}>
      ...
      <header className="border-b border-border/50 bg-white">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          <a href="/" className="flex items-center gap-2.5">
            ...
            <div><p className="text-sm font-bold text-foreground">Your Estimate</p><p className="text-[11px] text-muted-foreground/80">PlumbCore AI &bull; Premium Plumbing</p></div>
          </a>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 sm:py-10 pb-16">
        {!isExpired && data && (
          <div className="space-y-6">
            ...
            <Card className="border-border shadow-sm"><CardContent className="p-5"><p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">Your Plumber</p><div className="flex items-center gap-4"><Avatar className="w-14 h-14"><AvatarFallback className="bg-gradient-to-br from-primary to-blue-bright text-white text-sm font-bold">MT</AvatarFallback></Avatar><div className="flex-1"><p className="text-base font-bold text-foreground">{plumber.name}</p><div className="flex items-center gap-2 text-xs text-muted-foreground mt-0.5"><span className="flex items-center gap-0.5"><Star className="w-3.5 h-3.5 fill-amber-400 text-amber-400" />{plumber.rating}</span><span>&bull;</span><span>{plumber.jobs.toLocaleString()} jobs</span><span>&bull;</span><span>Lic. {plumber.license}</span></div></div><a href="tel:+155****4567" className="w-10 h-10 rounded-full bg-blue-tint text-primary flex items-center justify-center hover:bg-blue-tint active:scale-90 transition-all"><Phone className="w-5 h-5" /></a></div></CardContent></Card>
            <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="rounded-2xl bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-200 p-5 text-center">
                    ...
                    <div className="space-y-2">
                      ...
                      <button className="w-full h-11 rounded-xl bg-white border border-border text-muted-foreground font-medium text-sm shadow-sm transition-all active:scale-[0.97] flex items-center justify-center gap-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                <button className="rounded-xl border border-border text-muted-foreground text-xs py-3 h-11 font-medium active:scale-[0.97] transition-all"><Calendar className="w-4 h-4 inline mr-1.5" />Reschedule</button>
                <button className="rounded-xl border border-border text-muted-foreground text-xs py-3 h-11 font-medium active:scale-[0.97] transition-all"><MessageCircle className="w-4 h-4 inline mr-1.5" />Ask</button>
                <button className="rounded-xl border border-border text-red-500 text-xs py-3 h-11 font-medium hover:text-red-600 active:scale-[0.97] transition-all"><X className="w-4 h-4 inline mr-1.5" />Cancel</button>
              </div>
            </div>
            <Card className="border-border shadow-sm"><CardContent className="p-5"><p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-4">Progress</p><StatusTimeline currentStatus={status} /></CardContent></Card>
            <p className="text-center text-xs text-muted-foreground/80 flex items-center justify-center gap-1.5"><Shield className="w-3.5 h-3.5" /> Secured by PlumbCore AI &bull; Your data is encrypted</p>
          </div>
        )}
        {isExpired && (
          <Card className="border-red-200 bg-red-50 shadow-sm"><CardContent className="p-8 text-center"><div className="w-12 h-12 rounded-full bg-red-100 flex items-center justify-center mx-auto mb-3"><AlertTriangle className="w-6 h-6 text-red-500" /></div><h3 className="text-base font-bold text-foreground mb-1">This link has expired</h3><p className="text-sm text-muted-foreground mb-5">Quote links are valid for 7 days.</p><button onClick={() => router.push('/quote/plumbcore')} className="h-11 px-6 rounded-xl bg-amber-400 hover:bg-amber-500 text-foreground font-bold transition-all active:scale-[0.97]">Request new quote</button></CardContent></Card>
        )}
        {!isExpired && !data && <p className="text-center text-sm text-muted-foreground py-12">No estimate data found for this link.</p>}
      </div>
