    ...
    <div className="min-h-screen flex flex-col lg:flex-row">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-blue-tint via-blue-tint2 to-blue-tint2 text-foreground p-12 xl:p-16 flex-col justify-between relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <p className="text-muted-foreground text-sm max-w-md leading-relaxed">{t('auth.login.brandSubtitle')}</p>
        ...
        <div className="relative z-10 flex items-center gap-4"><div className="flex -space-x-2">{[...Array(4)].map((_, i) => <div key={i} className="w-8 h-8 rounded-full bg-muted border-2 border-white flex items-center justify-center text-[10px] font-bold text-muted-foreground">P{i+1}</div>)}</div><div><p className="text-sm font-medium text-foreground">{t('auth.login.trusted')}</p><p className="text-xs text-muted-foreground/80">{t('auth.login.rating')}</p></div></div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center bg-white p-6 sm:p-10 min-h-screen lg:min-h-0">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-6 text-center">
            <h1 className="text-2xl font-semibold tracking-tight text-foreground">{t('auth.login.title')}</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="relative my-3">
                ...
                <div className="relative flex justify-center">
                  <span className="bg-white px-3 text-xs text-muted-foreground/80">{t('auth.login.orContinue')}</span>
                </div>
              </div>
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">{t('auth.login.noAccount')}{' '}<a href="/signup" className="font-medium text-primary hover:text-primary/80 transition-colors">{t('auth.login.signUp')}</a></p>
        </div>
      </div>
    </div>
  );
